...
class followUpDetailSuperAdminPage extends StatefulWidget {
  ...
  const followUpDetailSuperAdminPage({Key key}) : super(key: key);

  @override
  _followUpDetailSuperAdminPageState createState() => _followUpDetailSuperAdminPageState();
}

class _followUpDetailSuperAdminPageState extends State<followUpDetailSuperAdminPage> {
  var apiURLFollowUp = "http://192.168.43.149:8080/api/log-follow-up/get";

  Future<FollowUp> functionListFollowUp() async {
    var body = jsonEncode({
      "logReportId" : followUpDetailSuperAdminPage.logReportId
    });
  ...
  functionSetTicketStatus() {
    if(followUpDetailSuperAdminPage.ticketStatusId == '0') {
      setState(() {
        followUpDetailSuperAdminPage.ticketStatus = 'Waiting for Support';
      });
    } else if(followUpDetailSuperAdminPage.ticketStatusId == '1') {
      setState(() {
        followUpDetailSuperAdminPage.ticketStatus = 'In Progress';
      });
    } else if(followUpDetailSuperAdminPage.ticketStatusId == '2') {
      setState(() {
        followUpDetailSuperAdminPage.ticketStatusId = 'Done';
      });
    } else {
      setState(() {
        followUpDetailSuperAdminPage.ticketStatus = 'Null';
      });
    }
  }
        ...
        appBar: AppBar(
          ...
          title: Row(
              children: <Widget>[
                ...
                Text(followUpDetailSuperAdminPage.ticketNumber.toString()),
                Text(" - "),
                Text(followUpDetailSuperAdminPage.ticketStatus)
              ]
          ),
          actions: <Widget>[
            IconButton(
              icon: Icon(Icons.add),
              onPressed: (){
                if(followUpDetailSuperAdminPage.ticketStatus == "Done") {
                  showDialog(
                      context: context,
                  ...
                  Navigator.of(context).push(PageTransition(child: addFollowUpSuperAdminPage(), type: PageTransitionType.bottomToTop));
                }
              },
            )
          ],
                    ...
                    child: Column(
                      children: <Widget>[
                        ...
                        Container(
                          alignment: Alignment.topLeft,
                          child: Text(
                            followUpDetailSuperAdminPage.ticketDetail.toString(),
                            style: TextStyle(
                                color: Colors.black,
                                fontSize: 17
                            ),
