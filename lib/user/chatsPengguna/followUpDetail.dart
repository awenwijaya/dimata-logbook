...
import 'package:dimata_logbook/user/chatsPengguna/addFollowUp.dart';
...
class followUpDetailPage extends StatefulWidget {
  ...
  const followUpDetailPage({Key key}) : super(key: key);
...
class _followUpDetailPageState extends State<followUpDetailPage> {
  ...
  functionSetTicketStatus() {
    if(followUpDetailPage.ticketStatusId == '0') {
      setState(() {
        followUpDetailPage.ticketStatus = 'Waiting for Support';
      });
    } else if(followUpDetailPage.ticketStatusId == '1') {
      setState(() {
        followUpDetailPage.ticketStatus = 'In Progress';
      });
    } else if(followUpDetailPage.ticketStatusId == '2') {
      setState(() {
        followUpDetailPage.ticketStatus = 'Done';
      });
    } else {
      setState(() {
        followUpDetailPage.ticketStatus = 'Null';
      });
    }
  }
        ...
        appBar: AppBar(
          ...
          title: Row(
            children: <Widget>[
              ...
              Text(followUpDetailPage.ticketNumber.toString()),
              Text(" - "),
              Text(followUpDetailPage.ticketStatus)
            ],
          ),
          actions: <Widget>[
            IconButton(
              icon: Icon(Icons.add),
              onPressed: (){
                if(followUpDetailPage.ticketStatus == "Done") {
                  showDialog(
                    context: context,
                  ...
                  Navigator.of(context).push(PageTransition(child: addFollowUpPage(), type: PageTransitionType.rightToLeft));
                }
              },
            )
          ],
                    ...
                    child: Column(
                      children: <Widget>[
                        ...
                        Container(
                          alignment: Alignment.topLeft,
                          child: Text(
                            followUpDetailPage.ticketDetail.toString(),
                            style: TextStyle(
                                color: Colors.black,
                                fontSize: 17
                            ),
